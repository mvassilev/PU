...
int main ( );
unsigned char *julia_picture ( int w, int h, float xl, float xr, float yb, float yt );
int julia_pixel ( int w, int h, float xl, float xr, float yb, float yt, int i, int j );
...
/*
  ...
  Discussion:
    ...
    of WxH pixels in R.  Each pixel is associated with a complex value Z.
    We carry out up to 200 steps of the iteration for each pixel.  
    If the iterated value exceeds 1000 at any time, the pixel is not 
    in the Julia set.
...
/*
  Compute the RGB value of each pixel.
*/
  t1 = omp_get_wtime ( );
  rgb = julia_picture ( w, h, xl, xr, yb, yt );
  t2 = omp_get_wtime ( ) - t1;
  printf ( "  Using %d threads max, %g seconds\n", omp_get_max_threads ( ), t2 );
/*
  Write the RGB data to a TARGA file.
*/
  tga_write ( w, h, rgb, "julia.tga" );
/*
  Free memory.
*/
  free ( rgb );
...
/*
  Purpose:
    ...
    JULIA_PICTURE creates an image of a Julia set.
...
# pragma omp for
    ...
    for ( j = 0; j < h; j++ )
    {
      for ( i = 0; i < w; i++ )
      {
        juliaValue = julia_pixel ( w, h, xl, xr, yb, yt, i, j );

        k = 3 * ( j * w + i );
/*
  In the TGA format, the ordering of colors is B/G/R!
*/
...
int julia_pixel ( int w, int h, float xl, float xr, float yb, float yt, int i, 
  int j )
...
/*
  Purpose:
    ...
    JULIA_PIXEL returns 1 if a pixel is in the Julia set.
  ...
  Parameters:
    ...
    Ouput, int JULIA_PIXEL, is 1 if the point is in the Julia set.
*/
{
  float ai;
  float ar;
...
/*
  ...
  Discussion:
    ...
    The ImageMagick program "display" can display a TGA file.
