// g++ -g -framework OpenCL m_cl.cpp -o m_cl
...
// Compute Stride to Avoid Floating Point Errors
    ...
    for(unsigned int i = 0; i < nreals / stride; i++)
        ans_grid.push_back(std::vector<int>(ans + (i * stride), ans + (i + 1) * stride));

    // Print Statistics
    //std::cout << "CPU Time: " << (float) (cpu_end - cpu_begin) / CLOCKS_PER_SEC << "s" << std::endl;

    // Write the Mandelbrot Set to File
    ppm_draw("mandelbrot23.ppm", ans_grid);
